...
def run_experiment(alg_name, specs_type, max_steps, fully_obs, network, learning_rate, 
					...
					visual, mem, random_walker,trainSz, seedB, complexT, loss, cnn_type,
					...
					loaded_test_avg, loaded_train_avg, AE, num_times = 1, resolution = 9):
	"""
	...
	learner = NSAgent(alg_name, network, batch_size, learning_rate, num_neurons, num_layers, 
						multiModal, TBPTT, rnn_type, mem_type, mapType, loss,
						text_mode, AE, cnn_type)
...
if __name__ == "__main__":
	...
	parser.add_argument('--syntax', default='TTL1', type=str, 
	...
	parser.add_argument('--AE',  default = False, 
						action ='store_true',
						help='whether the model uses a self-supervised reconstruction loss')
	parser.add_argument('--multimodal',  default = False, 
						action ='store_true',
						help='This parameter tells if a MultiModal network is \
								used, note that this mode is not supported for most cnns and belongs to\
								follow-up research')
	...
	parser.add_argument('--cnn_type', default='cnn', type=str, 
						help='This parameter indicates which kinf of cnn network to \
						use. The options are: ' + str(cnn_types))
	...
	run_experiment(alg_name = args.algorithm, specs_type = args.syntax, 
					...
					AE = args.AE, cnn_type = args.cnn_type)